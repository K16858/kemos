...
KERNEL_OBJS := $(KERNEL_DIR)/main.o $(KERNEL_DIR)/font.o $(KERNEL_DIR)/terminus_data.o \
	...
	$(KERNEL_DIR)/pic.o $(KERNEL_DIR)/keyboard.o
...
$(KERNEL_DIR)/main.o: $(KERNEL_DIR)/main.cpp $(KERNEL_DIR)/console.hpp $(KERNEL_DIR)/interrupt.hpp $(KERNEL_DIR)/pic.hpp $(KERNEL_DIR)/keyboard.hpp $(KERNEL_DIR)/graphics.hpp $(KERNEL_DIR)/frame_buffer_config.hpp devenv/buildenv.sh
	@bash -c "source devenv/buildenv.sh && \
		clang++ \$$CPPFLAGS --target=x86_64-elf -O2 -Wall -g --std=c++17 \
		-ffreestanding -mno-red-zone -fno-exceptions -fno-rtti \
...
$(KERNEL_DIR)/pic.o: $(KERNEL_DIR)/pic.cpp $(KERNEL_DIR)/pic.hpp $(KERNEL_DIR)/asmfunc.h devenv/buildenv.sh
	@bash -c "source devenv/buildenv.sh && \
		clang++ \$$CPPFLAGS --target=x86_64-elf -O2 -Wall -g --std=c++17 \
		-ffreestanding -mno-red-zone -fno-exceptions -fno-rtti \
...
$(KERNEL_DIR)/keyboard.o: $(KERNEL_DIR)/keyboard.cpp $(KERNEL_DIR)/keyboard.hpp $(KERNEL_DIR)/console.hpp $(KERNEL_DIR)/asmfunc.h $(KERNEL_DIR)/pic.hpp devenv/buildenv.sh
	@bash -c "source devenv/buildenv.sh && \
		clang++ \$$CPPFLAGS --target=x86_64-elf -O2 -Wall -g --std=c++17 \
		-ffreestanding -mno-red-zone -fno-exceptions -fno-rtti \
		-I$(KERNEL_DIR) -c $(KERNEL_DIR)/keyboard.cpp -o $(KERNEL_DIR)/keyboard.o"
